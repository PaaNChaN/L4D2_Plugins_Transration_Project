...
public Action:CmdBonus(client, args)
{
	...
	{
		CPrintToChat(client, "%t", "Print_CmdBonus_Lite", PLUGIN_TAG, InSecondHalfOfRound() + 1, RoundToFloor(fHealthBonus + fDamageBonus + fPillsBonus), CalculateBonusPercent(fHealthBonus + fDamageBonus + fPillsBonus, fMapHealthBonus + fMapDamageBonus + fMaxPillsBonus));
		// R#1 Bonus: 556 <69.5%>
	}
	else
	{
		if (InSecondHalfOfRound())
		{
			CPrintToChat(client, "%t", "Print_CmdBonus_SecondRound", PLUGIN_TAG, RoundToFloor(fSurvivorBonus[0]), CalculateBonusPercent(fSurvivorBonus[0]));
		}
		CPrintToChat(client, "%t", "Print_CmdBonus", PLUGIN_TAG, InSecondHalfOfRound() + 1, RoundToFloor(fHealthBonus + fDamageBonus + fPillsBonus), CalculateBonusPercent(fHealthBonus + fDamageBonus + fPillsBonus, fMapHealthBonus + fMapDamageBonus + fMaxPillsBonus), CalculateBonusPercent(fHealthBonus, fMapHealthBonus), CalculateBonusPercent(fDamageBonus, fMapDamageBonus), CalculateBonusPercent(fPillsBonus, fMaxPillsBonus));
...
public Action:CmdMapInfo(client, args)
{
	...
	CPrintToChat(client, "%t", "Print_CmdMapInfo_Title", iTeamSize, iTeamSize);
	CPrintToChat(client, "%t", "Print_CmdMapInfo_Distance", iMapDistance);
	CPrintToChat(client, "%t", "Print_CmdMapInfo_TotalBonus", RoundToFloor(fTotalBonus));
	CPrintToChat(client, "%t", "Print_CmdMapInfo_HealthBonus", RoundToFloor(fMapHealthBonus), CalculateBonusPercent(fMapHealthBonus, fTotalBonus));
	CPrintToChat(client, "%t", "Print_CmdMapInfo_DamageBonus", RoundToFloor(fMapDamageBonus), CalculateBonusPercent(fMapDamageBonus, fTotalBonus));
	CPrintToChat(client, "%t", "Print_CmdMapInfo_PillsBonus", iPillWorth, RoundToFloor(fMaxPillsBonus), CalculateBonusPercent(fMaxPillsBonus, fTotalBonus));
	CPrintToChat(client, "%t", "Print_CmdMapInfo_Tiebreaker", iPillWorth);
...
public Action:PrintRoundEndStats(Handle:timer) 
{
	...
	{
		CPrintToChatAll("%t", "Print_RoundEndStats_TIEBREAKER", PLUGIN_TAG, iSiDamage[0], iSiDamage[1]);
		if (iSiDamage[0] == iSiDamage[1])
		{
