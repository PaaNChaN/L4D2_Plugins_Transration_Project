...
public TankSpawn_Event(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	{
		SetGlobalTransTarget(tank);
		CPrintToChat(tank, "%t", "rockSelector");
		CPrintToChat(tank, "%t", "twoHandOverhand");
		CPrintToChat(tank, "%t", "underhandThrow");
		CPrintToChat(tank, "%t", "oneHandOverhand");
	}
}
