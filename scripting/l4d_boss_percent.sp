...
public Action:PrintStuff(Handle:timer, any:client)
{
	if(IsValidClient(client))
	{
		...
		{
			if (iTankPercent)
				CPrintToChat(client, "%t", "tank_percent", iTankPercent);
			else
				CPrintToChat(client, "%t", "tank_none");
		}

		if(GetConVarBool(hCvarWitchPercent))
		{
			if (iWitchPercent)
				CPrintToChat(client, "%t", "witch_percent", iWitchPercent);
			else
				CPrintToChat(client, "%t", "witch_none");
		}
	}	
}
