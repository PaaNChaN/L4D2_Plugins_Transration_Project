...
public Action:Tank_Cmd(client, args)
{
    ...
    if (tankClientId != -1)
    {
        ...
        if (L4D2Team:GetClientTeam(client) == L4D2Team:L4D2Team_Infected || IsClientCaster(client))
        {
            ...
            {
                CPrintToChat(client, "%t", "ltc_TankMessage_you");
            }
            else
            {
                CPrintToChat(client, "%t", "ltc_TankMessage", tankClientName);
            }
        }
        else if ((GetConVarBool(hTankPrint) && L4D2Team:GetClientTeam(client) == L4D2Team:L4D2Team_Survivor) || L4D2Team:GetClientTeam(client) == L4D2Team:L4D2Team_Spectator)
        {
            CPrintToChat(client, "%t", "ltc_TankMessage", tankClientName);
        }
    }
...
{    
    ...
    {
        PrintHintText(tank_index, "%t", "lototb_ragemeter_hint");
        for (new i = 1; i <= MaxClients; i++) 
        {
            ...
            if (tank_index == i) 
            {
                CPrintToChat(i, "%t", "lototb_ragemeter_chat_you");
            }
            else 
            {
                CPrintToChat(i, "%t", "lototb_ragemeter_chat", tank_index);
            }
        }
...
public outputTankToAll()
{
    ...
    if (tankClientId != -1)
    {
        ...
        {
            CPrintToChatAll("%t", "ltc_TankMessage", tankClientName);
        }
        else
        {
            for (new i = 1; i <= MaxClients; i++) 
            {
                ...
                {
                    if (L4D2Team:GetClientTeam(i) == L4D2Team:L4D2Team_Survivor)
                    {
                        continue;
                    }
                    ...
                    if (i == tankClientId)
                    {
                        CPrintToChat(i, "%t", "ltc_TankMessage_you");
                    }
                    else
                    {
                        CPrintToChat(i, "%t", "ltc_TankMessage", tankClientName);
                    }
                }
            }
        }
