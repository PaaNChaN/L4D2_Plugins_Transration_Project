...
PrintRemainingHealth()
{
	...
	CPrintToChatAll("%t", "prh_remaining_health", name, g_iLastTankHealth);
}
...
PrintTankDamage()
{
	...
	if (!bPrintedHealth)
	{
		for (new i = 1; i <= MaxClients; i++)
		{
			if(g_iWasTank[i] > 0)
			{
				...
				CPrintToChatAll("%t", "ptd_tankdmg", name);
				g_iWasTank[i] = 0;
			}
			else if(g_iWasTankAI > 0) 
				CPrintToChatAll("%t", "ptd_tankdmg", "AI");
			g_iWasTankAI = 0;
		}
	}
	...
	for (new k; k <= survivor_index; k++)
	{
		...
		for (new i = 1; i <= MaxClients; i++)
		{
    		if (IsClientInGame(i))
    		{
				CPrintToChat(i, "%t", "ptd_tank_surdmg", damage, percent_damage, client);
			}
		}
	}
}
